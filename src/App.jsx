...
const migrateLoadedDataset = (s) => {
  ...
  Object.values(legacyGroups).forEach((group, gi) => {
    const normType = /frigo|refriger/i.test(group.typeText) ? 'Refrigerator' : /clos|armad|closet/i.test(group.typeText) ? 'Closet' : 'Freezer';
...
export default function App() {
  const createEmptyTest = (id, num, customType = 'plate-96') => {
    const baseTest = {
      id, name: `Test ${num}`, date: new Date().toISOString().split('T')[0], instanceName: '', testCategory: 'Activity', type: customType,
      storageType: '', storageLabel: '', storageIndex: null, comments: '', images: [], documents: [], plan: [], linkedProtocolId: '',
      cellLines: [], customFieldValues: {}, selectedCompounds: [], compound: ''
    };
    ...
    if (customType.startsWith('plate')) {
      ...
      return {
        ...
        compounds: Array(dim?.cols || 12).fill(''), rowCompounds: Array(dim?.rows || 8).fill(''), cellConfig: defCell,
    ...
    if (customType === 'nmr') {
      return {
        ...baseTest, proteinSequence: '', selectedNuclei: ['H', 'C', 'N'], chemicalShifts: {}, nmrSpectraImages: [],
        moleculeName: '', experimentDate: '', concentration: '', solvent: '', saltConcentration: '', temperature: '', otherMolecule: '', ratio: '', tableMode: 'backbone', compound: ''
      };
    } 
    ...
    if (customType === 'cd') {
      return {
        ...baseTest, compound: '', experimentDate: '', concentration: '', solvent: '', saltConcentration: '', temperature: '', buffer: '', pathLength: '1', otherMolecule: '', ratio: '',
        wavelengthData: '', spectraColumns: [], structureComposition: { 'α-Helix': 30, 'β-Sheet': 20, 'Turn': 10, 'Random Coil': 40 },
        chartCfg: { yMin: '', yMax: '', xMin: '190', xMax: '260', fontSize: 12, lineWidth: 2 }
      };
    }
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      auth.onAuthStateChanged(currentUser => {
        if (currentUser) { setUser(currentUser); setNeedsLogin(false); setIsCloudReady(true); } 
        else { setNeedsLogin(true); setIsCloudReady(true); }
      });
    };
    initAuth();
  }, []);
  ...
  const handleManualLogin = async () => {
    ...
    try { await auth.signInWithPopup(provider); } catch (e) { console.error("Errore login:", e); }
  };
  ...
  useEffect(() => {
    ...
    if (sharedDatasetId && isCloudReady && db && !currentDatasetId) {
      db.collection(`artifacts/${appId}/public/data/datasets`).doc(sharedDatasetId).get()
        .then(doc => { if (doc.exists) { const dset = { id: doc.id, ...doc.data() }; openDataset(dset); } })
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        if (db && user) {
          ...
          await docRef.set(updatedPayload, { merge: true }).then(() => { setSaveStatus('saved'); setSaveErrorMsg(''); })
            .catch(err => { setSaveStatus('error'); setSaveErrorMsg(err.message); });
        } else {
          let stored = [];
  ...
  }, [tests, datasetTitle, datasetSubtitle, customCmpds, customCellLines, customConc, cmpColors, testCategories, protocolCategories, datasetProtocols, customFields, storages, isCloudReady, appView, currentDatasetId, user]);
  ...
  const deleteEmptyDatasets = async () => {
        ...
        try {
          if (dset.title === 'New Dataset' || dset.title === 'Untitled Dataset') {
            ...
            if (s && s.tests && s.tests.length === 1) {
              const t = s.tests[0];
              if (t.name === 'Test 1' && !t.instanceName && !t.comments && (!t.images || t.images.length === 0)) return true;
            }
          }
    ...
    if (emptyDatasets.length === 0) { setDialog({ type: 'alert', title: 'Clean Up', message: 'No empty datasets found.' }); return; }
    setDialog({
      type: 'confirm', title: 'Delete Empty Datasets', message: `Are you sure you want to delete ${emptyDatasets.length} empty dataset(s)?`,
      onConfirm: async () => {
        if (db && user) {
          try {
            const batch = db.batch();
            emptyDatasets.forEach(dset => { const docRef = db.collection(`artifacts/${appId}/public/data/datasets`).doc(dset.id); batch.delete(docRef); });
            await batch.commit();
          } catch (e) { console.error("Batch delete failed", e); setDialog({ type: 'alert', title: 'Error', message: 'Cloud deletion failed.' }); }
        } else {
          let stored = []; try { stored = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY) || '[]'); } catch (e) { }
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-slate-100 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl flex flex-col items-center max-w-sm border border-slate-200 text-center">
          ...
          <p className="text-slate-500 mb-8 text-sm leading-relaxed">Per ragioni di sicurezza e per sincronizzare i tuoi dati di laboratorio sul Cloud, il browser richiede un'azione manuale per il login.</p>
          <button onClick={handleManualLogin} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full shadow-lg transition-transform hover:scale-105 w-full flex items-center justify-center gap-2">
    ...
    <div className="w-full relative flex flex-col h-screen overflow-hidden bg-slate-50">
      <style>{`
        @media print {
          ...
          body, html, #root { background: white !important; height: auto !important; min-height: 100% !important; overflow: visible !important; color: black !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          ...
          .avoid-break, table, tr, thead, tbody, img, svg, canvas, figure { break-inside: avoid !important; page-break-inside: avoid !important; }
          ...
          #notebook-report-container > * { break-inside: avoid !important; page-break-inside: avoid !important; margin-top: 0.4cm; margin-bottom: 0.4cm; }
        }
      `}</style>
      ...
      <StorageModals storageModal={storageModal} setStorageModal={setStorageModal} storages={storages} setStorages={setStorages} moveModal={moveModal} setMoveModal={setMoveModal} tests={tests} setTests={setTests} />
            ...
            <div className="bg-white p-4 md:p-8 rounded-2xl shadow-xl border border-slate-200">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 border-b border-slate-100 pb-4 gap-4">
                ...
                <div className="flex flex-wrap gap-2 w-full md:w-auto">
                  <button onClick={deleteEmptyDatasets} className="bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 font-bold py-2 px-4 rounded-lg shadow-sm transition-colors flex-1 md:flex-none items-center justify-center gap-2 cursor-pointer text-sm">🗑️ Delete Empty</button>
        ...
        <div className="flex flex-col md:flex-row h-screen w-full overflow-hidden">
          ...
          <div className={`bg-white border-r border-slate-200 flex flex-col shadow-sm z-50 shrink-0 no-print transition-all duration-300 absolute md:relative h-full ${isSidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0 md:w-16 items-center'}`}>
            <div className={`p-4 border-b border-slate-200 flex items-center gap-2 ${isSidebarOpen ? 'justify-between' : 'flex-col justify-center'}`}>
              ...
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-slate-400 hover:text-slate-600 transition-colors text-lg" title="Toggle Sidebar">
                {isSidebarOpen ? '⮜' : '☰'}
              </button>
            </div>
            {isSidebarOpen && (
            ...
            <nav className={`flex-1 overflow-y-auto py-4 flex flex-col gap-1 ${isSidebarOpen ? 'px-2' : 'px-1 items-center'}`}>
              {[
                ...
                <button key={nav.id} onClick={() => { setCurrentModule(nav.id); if (window.innerWidth < 768) setIsSidebarOpen(false); }} title={!isSidebarOpen ? nav.label : ''} className={`flex items-center gap-3 py-2 rounded-lg text-sm transition-all text-left ${isSidebarOpen ? 'px-3 w-full' : 'px-0 w-10 justify-center'} ${currentModule === nav.id ? 'bg-blue-50 text-blue-700 font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}>
            ...
            <div className={`p-4 border-t border-slate-200 flex flex-col gap-2 ${!isSidebarOpen ? 'items-center px-1' : ''}`}>
              <div className={`flex ${isSidebarOpen ? 'gap-2' : 'flex-col gap-2 w-full'}`}>
                ...
                <button onClick={exportHTML} className={`flex-1 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-bold py-1.5 rounded text-xs shadow-sm transition-colors ${!isSidebarOpen ? 'py-2 px-0 text-[10px]' : ''}`} title="Save HTML">
          ...
          <div className="flex-1 flex flex-col bg-slate-50 h-full overflow-hidden relative">
              ...
              <div className="p-4 md:p-8 h-full overflow-y-auto custom-scrollbar bg-slate-50">
                <div className="max-w-6xl mx-auto">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 border-b border-slate-200 pb-4 gap-4">
                    ...
                    <button onClick={handlePrint} className="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold py-2 px-4 rounded-lg text-sm transition-colors flex items-center gap-2 shadow-sm no-print w-full md:w-auto justify-center">🖨️ Print / Save PDF</button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {[
                      ...
                      <button key={mod.id} onClick={() => setCurrentModule(mod.id)} className="bg-white p-5 md:p-6 rounded-lg border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-400 transition-all text-left group no-print">
                  ...
                  <div className="w-full md:w-80 bg-white border border-slate-200 rounded-xl p-4 shadow-sm shrink-0 h-fit no-print">
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      ...
                      {Array.from({ length: totalDays }, (_, i) => {
                        ...
                        return (
                          <button key={i} onClick={() => setCalFilterDate(isSel ? null : dateStr)} className={`text-[11px] py-1.5 rounded-md transition-all font-medium ${isSel ? 'bg-blue-600 text-white shadow-md scale-105' : hasTask ? 'bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100' : 'text-slate-600 hover:bg-slate-100'}`}>
                            {i + 1}
                          </button>
                        );
                      })}
                            ...
                            <div className="flex flex-col gap-2">
                              {tasks.map((t, idx) => (
                                <div key={idx} className="flex flex-col md:flex-row md:items-center gap-2 md:gap-3 bg-slate-50 p-3 border border-slate-200 rounded-lg group hover:border-blue-300 transition-colors">
                                  <button onClick={() => jumpToTest(t.testId)} className="text-xs font-bold bg-blue-100 hover:bg-blue-200 text-blue-800 px-3 py-1.5 rounded-md transition-colors whitespace-nowrap shadow-sm self-start md:self-auto">{t.testName}</button>
            ...
            {currentModule === 'storage' && (
              <StorageList storages={storages} tests={tests} setStorageModal={setStorageModal} setActiveStorageId={setActiveStorageId} setCurrentModule={setCurrentModule} handlePrint={handlePrint} />
            )}

            {currentModule === 'storage-detail' && (
              <StorageDetail storages={storages} activeStorageId={activeStorageId} tests={tests} setTests={setTests} setCurrentModule={setCurrentModule} handlePrint={handlePrint} jumpToTest={jumpToTest} setMoveModal={setMoveModal} createEmptyTest={createEmptyTest} setActiveTestId={setActiveTestId} />
            )}
                ...
                <div className="p-4 md:p-6 h-full flex flex-col">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4 border-b border-slate-200 pb-4">
                    ...
                    <div className="flex flex-wrap gap-2 no-print w-full md:w-auto">
                      <button onClick={handlePrint} className="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center gap-2 shadow-sm flex-1 md:flex-none">🖨️ PDF</button>
                      <button onClick={() => { const id = 't' + Date.now(); setTests(prev => [...prev, createEmptyTest(id, prev.length + 1, 'plate-96')]); setActiveTestId(id); setCurrentModule('active-test'); }} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-sm text-sm transition-colors flex-1 md:flex-none">+ Plate</button>
                      <button onClick={() => { const id = 't' + Date.now(); setTests(prev => [...prev, createEmptyTest(id, prev.length + 1, 'nmr')]); setActiveTestId(id); setCurrentModule('active-test'); }} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded shadow-sm text-sm transition-colors flex-1 md:flex-none">+ NMR</button>
                      <button onClick={() => { const id = 't' + Date.now(); setTests(prev => [...prev, createEmptyTest(id, prev.length + 1, 'cd')]); setActiveTestId(id); setCurrentModule('active-test'); }} className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded shadow-sm text-sm transition-colors flex-1 md:flex-none">+ CD</button>
                    </div>
                  </div>
                    ...
                    {showCatMgr && (
                      <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col gap-3">
                        ...
                        <div className="flex flex-col md:flex-row gap-2">
                          ...
                          <button onClick={() => { const v = newCatInput.trim(); if (v && !testCategories.includes(v)) { setTestCategories([...testCategories, v]); setExpandedGroups(p => ({ ...p, newTestCatInput: '' })); } }} className="bg-blue-600 text-white font-bold px-4 py-2 rounded text-sm shadow-sm hover:bg-blue-700 transition-colors">Add Category</button>
                        </div>
                      </div>
                    )}
                  </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {filteredTests.map(test => (
                          <div key={test.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:border-blue-400 cursor-pointer transition-all flex flex-col group relative" onClick={() => { setActiveTestId(test.id); setCurrentModule('active-test'); }}>
                            <button onClick={(e) => { e.stopPropagation(); if (window.confirm(`Eliminare definitivamente il test "${test.name}"?`)) { setTests(prev => prev.filter(t => t.id !== test.id)); } }} className="absolute top-3 right-10 text-slate-300 hover:text-red-500 text-xl opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity no-print z-10" title="Elimina Test">&times;</button>
                  ...
                  <div className="p-4 md:p-6 h-full flex flex-col bg-white">
                    ...
                    <div className="flex-1 flex flex-col lg:flex-row gap-6 overflow-y-auto lg:overflow-hidden">
                      <div className="flex-1 flex flex-col h-auto lg:h-full min-h-[300px]">
                        ...
                        <RichTextEditor value={activeProtocol.content || ''} onChange={val => setDatasetProtocols(datasetProtocols.map(p => p.id === activeProtocol.id ? { ...p, content: val } : p))} placeholder="Write the detailed protocol steps here. You can paste images directly..." />
                        <div className="mt-6 border-t border-slate-100 pt-4 no-print shrink-0">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {tests.filter(t => t.linkedProtocolId === activeProtocol.id || (Array.isArray(t.linkedProtocolIds) && t.linkedProtocolIds.includes(activeProtocol.id))).length === 0 && (
                              <span className="text-sm text-slate-400 italic">No tests are currently linked to this protocol.</span>
                            )}
                            {tests.filter(t => t.linkedProtocolId === activeProtocol.id || (Array.isArray(t.linkedProtocolIds) && t.linkedProtocolIds.includes(activeProtocol.id))).map(t => (
                              <button key={t.id} onClick={() => { setActiveTestId(t.id); setCurrentModule('active-test'); }} className="text-xs font-bold text-blue-700 bg-blue-50 border border-blue-200 hover:bg-blue-100 px-3 py-1.5 rounded-lg shadow-sm transition-colors flex items-center gap-1">
                        ...
                        <div className="flex flex-col gap-2">
                          <button onClick={() => {
                            ...
                            if (urlsText && urlsText.trim()) {
                              ...
                              const newLinks = urls.map((url, idx) => ({ id: Date.now().toString() + idx + Math.random(), name: 'Linked Resource', url: url }));
                              setDatasetProtocols(datasetProtocols.map(p => p.id === activeProtocol.id ? { ...p, links: [...(p.links || []), ...newLinks] } : p));
                            }
                    ...
                    {showProtoCatMgr && (
                      <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col gap-3">
                        ...
                        <div className="flex flex-col md:flex-row gap-2">
                          ...
                          <button onClick={() => { const v = newProtoCatInput.trim(); if (v && !protocolCategories.includes(v)) { setProtocolCategories([...protocolCategories, v]); setExpandedGroups(p => ({ ...p, newProtoCatInput: '' })); } }} className="bg-emerald-600 text-white font-bold px-4 py-2 rounded text-sm shadow-sm hover:bg-emerald-700 transition-colors">Add</button>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {filteredProtocols.map(proto => (
                          <div key={proto.id} className="bg-white border border-slate-200 rounded-xl p-4 md:p-5 shadow-sm hover:shadow-md hover:border-emerald-400 cursor-pointer transition-all flex flex-col group relative" onClick={() => setExpandedGroups(p => ({ ...p, activeProtoId: proto.id }))}>
                            <button onClick={(e) => { e.stopPropagation(); if (confirm("Delete this protocol?")) { setDatasetProtocols(datasetProtocols.filter(p => p.id !== proto.id)); } }} className="absolute top-3 right-3 text-slate-300 hover:text-red-500 text-lg md:opacity-0 group-hover:opacity-100 transition-opacity no-print" title="Delete Protocol">&times;</button>
                ...
                <div className="flex flex-col shrink-0 z-20 no-print">
                  <div className="bg-white border-b border-slate-200 px-4 md:px-6 py-4 flex flex-col lg:flex-row justify-between items-start lg:items-center shadow-sm gap-4">
                    <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 w-full lg:w-auto">
                      <button onClick={() => { if (isBox && activeTest.storageId) { setActiveStorageId(activeTest.storageId); setCurrentModule('storage-detail'); } else { setCurrentModule('tests'); } }} className="text-slate-400 hover:text-blue-600 transition-colors bg-slate-50 hover:bg-blue-50 p-2 rounded-lg shadow-sm border border-slate-200 self-start md:self-auto">◀ Back</button>
                      <div className="flex-1 w-full">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full lg:w-auto">
                      <button onClick={() => { if (window.confirm("Sei sicuro di voler eliminare definitivamente questo test?")) { setTests(prev => prev.filter(t => t.id !== activeTest.id)); setCurrentModule('tests'); } }} className="bg-red-50 text-red-600 hover:bg-red-100 hover:border-red-300 font-bold py-2 px-3 rounded-lg text-xs transition-colors border border-red-200 shadow-sm">🗑️ Elimina</button>
                      ...
                      {siblingTests.map((t, idx) => (
                        <button key={t.id} onClick={() => setActiveTestId(t.id)} className={`shrink-0 px-3 py-1.5 md:py-1 text-xs font-bold rounded-full transition-colors flex items-center gap-1.5 shadow-sm group ${activeTestId === t.id ? 'bg-blue-600 text-white' : 'bg-white text-blue-700 border border-blue-300 hover:bg-blue-100'}`}>
                          ...
                          {siblingTests.length > 1 && (
                            <span onClick={(e) => { e.stopPropagation(); if (confirm(`Delete instance ${t.instanceName || t.date}?`)) { setTests(prev => { const next = prev.filter(test => test.id !== t.id); if (activeTestId === t.id) setActiveTestId(next.find(x => x.name === t.name)?.id || next[0]?.id); return next; }); } }} className={`ml-1 px-1 opacity-100 md:opacity-0 group-hover:opacity-100 ${activeTestId === t.id ? 'text-blue-300 hover:text-white' : 'text-red-400 hover:text-red-600'}`}>&times;</span>
                          )}
                        </button>
                      ))}
                      <button onClick={handleDuplicateInstance} className="shrink-0 px-3 py-1.5 md:py-1 text-[10px] font-bold text-blue-600 border border-dashed border-blue-400 rounded-full hover:bg-blue-100 transition-colors bg-white shadow-sm ml-2">+ Add Timepoint/Copy</button>
                    </div>
                  )}
                </div>
              );
