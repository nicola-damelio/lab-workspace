...
// --- COMPONENTE EDITOR DI TESTO RICCO ---
...
const ChemicalShiftRanges = ({ sequence }) => {
  ...
  const uniqueResidues = [...new Set(sequence.map(r => r.char))];
...
// --- STRUTTURA CHIMICA 2D (Invariata) ---
...
// --- COMPONENTE GRAFICO CON ZOOM AVANZATO ---
const NMRPlotTemplateZoomable = ({ title, panelId, expandedPanel, setExpandedPanel, is1D, initialXDomain, initialYDomain, children }) => {
  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-xl shadow-sm p-4 flex flex-col ${isExpanded ? 'fixed inset-4 md:inset-10 z-[100] bg-white p-6 md:p-8 rounded-2xl shadow-2xl' : 'h-[500px]'}`}>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col xl:flex-row gap-6">
          {/* 4. TABELLA DI ASSEGNAZIONE DINAMICA (Combinata o Separata) */}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-4">
              {images.length === 0 ? (
                ...
                images.map((imgSrc, idx) => {
                  const directSrc = imgSrc; // Nota: getDirectImageUrl era nel codice originale, assicurati che sia importato o definito se necessario
                  return (
        ...
        {/* 6. VISUALIZZAZIONE SIMULAZIONI SPETTRI (Con Zoom) */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <NMRPlotTemplateZoomable title="Spectre 1D ¹H (Simulated)" panelId="1D_1H" expandedPanel={expandedPanel} setExpandedPanel={setExpandedPanel} is1D={true} initialXDomain={[0, 11]} initialYDomain={[0, 4.5]}>
                {({ xDomain, yDomain, isZoomed, refAreaLeft, refAreaRight }) => (
                  <>
                    <XAxis type="number" dataKey="x" domain={xDomain} reversed={true} ticks={isZoomed ? undefined : TICKS_1H} interval={0} tickLine={false} tick={<CustomXTick1H isZoomed={isZoomed}/>} label={{ value: '¹H (ppm)', position: 'insideBottom', offset: -25, fill: '#64748b' }} axisLine={{ stroke: '#cbd5e1' }} />
                    <YAxis type="number" dataKey="y" domain={yDomain} hide={true} />
              ...
              <NMRPlotTemplateZoomable title="Spectre COSY (Simulated)" panelId="cosy" expandedPanel={expandedPanel} setExpandedPanel={setExpandedPanel} is1D={false} initialXDomain={[0, 11]} initialYDomain={[0, 11]}>
                {({ xDomain, yDomain, isZoomed, refAreaLeft, refAreaRight, refAreaTop, refAreaBottom }) => (
                  <>
                    ...
                    <XAxis type="number" dataKey="x" domain={xDomain} reversed={true} ticks={isZoomed ? undefined : TICKS_1H} interval={0} tickLine={false} tick={<CustomXTick1H isZoomed={isZoomed}/>} label={{ value: '¹H F2 (ppm)', position: 'insideBottom', offset: -25, fill: '#64748b' }} />
                    <YAxis type="number" dataKey="y" domain={yDomain} reversed={true} ticks={isZoomed ? undefined : TICKS_1H} interval={0} tickLine={false} tick={<CustomYTick1H isZoomed={isZoomed}/>} label={{ value: '¹H F1 (ppm)', angle: -90, position: 'insideLeft', offset: -20, fill: '#64748b' }} />
