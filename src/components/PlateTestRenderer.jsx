...
/* ============================================================================
Plate renderer adapted to the shared shell.

Common sections (classification, compounds, conditions, protocols, agenda,
comments, images, notebook export) are rendered by TestShellRenderer.

Plate-specific sections are rendered inside one PlateAll component because
they share a lot of interactive state:
- Experiment Setup: format, units, dilution, custom concentrations
- Data: OD grid + visual plate map
- Fitting:
  - Error Management: normalization, fixed SD, outliers
  - Graphical Parameters: chart config, filters
  - Region charts / IC50 charts

IMPORTANT:
The shared shell uses compounds as metadata.
Plate uses activeTest.compounds as the column assignment array.
The wrapper at the bottom protects activeTest.compounds and maps shell
compound selection to selectedCompounds / compoundsSelected.
========================================================================== */
...
// ================= NOTEBOOK BUILDER =================
const buildPlateNotebookHtml = (checked, ctx) => {
  ...
  if (checked.cond) {
    ...
    html += `<p style="font-size: 12px; color: #475569; margin-bottom: 8px;">
      ...
      <b>Max Conc:</b> ${t.topConcStr || 'N/A'} ${t.unit || 'µM'} |
      <b>Dil. Factor:</b> ${t.dilFactorStr || 'N/A'} |
      <b>Ctrl OD:</b> ${t.ctrlODStr || 'N/A'} |
      <b>Blank OD:</b> ${model.bgOD.toFixed(4)} |
      <b>Cells/Well:</b> ${t.cellsSeeded || 'N/A'} |
      <b>Time:</b> ${t.timeBeforeRevelation || 'N/A'}h |
      <b>Protocols:</b> ${protTitles}
    </p>`;
  }

  if (checked.map && model.plotCmps.length > 0) {
    html += `<p style="font-size: 12px; color: #475569; margin-bottom: 8px;">
      <b>Compounds Tested:</b> ${model.plotCmps.join(', ')}
    </p>`;
  }
        ...
        if (c.fit) {
          html += `<tr>
            ...
            <td style="padding: 6px; border: 1px solid #e2e8f0;">
              ${c.fit.ic50.toFixed(3)} ± ${(c.fit.se * model.eScale).toFixed(3)}
            </td>
          </tr>`;
        }
      });
...
const PlateAll = ({ ctx }) => {
  ...
  const model = useMemo(
    ...
    [activePlateDim, plotCmps, processedByRegion, bgOD, eScale, unit]
  );
...
export const PlateTestRenderer = (props) => {
  ...
  - store them in compoundsSelected / selectedCompounds
  ...
  */
  const safeActiveTest = useMemo(() => {
    ...
    return {
      ...t,
      selectedCompounds: t.selectedCompounds || t.compoundsSelected || []
    };
  }, [props.activeTest]);
  ...
  const config = useMemo(
    () => ({
      ...PLATE_TAB_CONFIG,
      samplesLabel: PLATE_TAB_CONFIG.samples?.compoundLabel || 'Compound / Sample Label(s)',
      SetupSection: PlateAll,
      notebookChecks: [
        { id: 'cond', label: 'Experimental Conditions' },
        { id: 'map', label: 'Plate Map Summary' },
        { id: 'ic50', label: 'IC50 Results' }
      ],
      buildNotebookHtml: buildPlateNotebookHtml
    }),
    []
  );
  ...
  return (
    <TestShellRenderer
      ...
      testCategories={PLATE_TAB_CONFIG.categories}
    />
  );
};
