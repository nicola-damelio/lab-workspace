...
// --- COMPOUND MULTI SELECT / TYPEAHEAD ---
const CompoundMultiSelect = ({
    allCompounds = [],
    selected = [],
    onChange,
    onAddCustom,
    placeholder = 'Search or type a compound/sample name...'
}) => {
    ...
    const source = Array.from(new Set((allCompounds || []).filter(Boolean)));
    const q = query.trim().toLowerCase();
    const options = source
        .filter((c) => !selected.includes(c) && (!q || c.toLowerCase().includes(q)))
        .slice(0, 100);

    const exactMatch = source.some((c) => c.toLowerCase() === q);
    const canAddTyped = q && !exactMatch && !selected.includes(query.trim());
    ...
    const addValue = (val) => {
        ...
        if (onAddCustom && !source.includes(v)) {
            onAddCustom(v);
        }
    ...
    return (
        <div ref={wrapRef} className="relative">
            <div className="flex flex-wrap gap-2 mb-2">
                {selected.length === 0 && (
                    <span className="text-xs text-slate-400 italic">No compound selected.</span>
                )}

                {selected.map((c) => (
                    <span
                        key={c}
                        className="bg-blue-50 border border-blue-200 text-blue-800 text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-1 shadow-sm"
                    >
                        {c}
                        <button
                            type="button"
            ...
            <div className="flex gap-2">
                <input
                    ...
                    className="flex-1 border border-blue-300 rounded-md p-2 text-sm outline-none focus:border-blue-500 bg-white"
                />
                <button
                    type="button"
                    onClick={() => addValue(query)}
                    className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-md text-xs font-bold shadow-sm transition-colors"
                >
                    Add
                </button>
            </div>

            {open && (options.length > 0 || canAddTyped) && (
                <div className="absolute z-30 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-lg max-h-48 overflow-y-auto no-print">
                    {canAddTyped && (
                        <button
                            type="button"
                            onMouseDown={(e) => {
                            ...
                            className="w-full text-left px-3 py-2 text-sm font-bold text-blue-700 bg-blue-50 hover:bg-blue-100"
                        >
                            + Add "{query.trim()}"
                        </button>
                    )}
...
const ProtocolMultiSelect = ({ protocols = [], selectedIds = [], onChange, onJump }) => {
    const source = Array.isArray(protocols) ? protocols : [];
    const safeSelected = Array.isArray(selectedIds) ? selectedIds.filter(Boolean) : [];

    const available = source.filter((p) => p && p.id && !safeSelected.includes(p.id));

    const addId = (id) => {
        if (!id || safeSelected.includes(id)) return;
        ...
        <div className="flex flex-col gap-2">
                ...
                >
                    ...
                    {available.map((p) => (
                        <option key={p.id} value={p.id}>
                            {p.title || p.name || p.id} {p.category ? `(${p.category})` : ''}
                        </option>
                    ))}
                </select>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
                {safeSelected.length === 0 && (
                    <span className="text-xs text-slate-400 italic">No protocol linked.</span>
                )}

                {safeSelected.map((id) => {
                    const p = source.find((x) => x.id === id);
                    const label = p ? `${p.title || p.name || p.id} (${p.category || 'Protocol'})` : id;

                    return (
                        <span
                            key={id}
                            className="bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold px-2 py-1 rounded-lg flex items-center gap-2 shadow-sm"
                        >
                            ...
                            {p && onJump && (
                                <button
                                    type="button"
                                    onClick={() => onJump(id)}
                                    className="text-blue-600 hover:text-blue-800 font-bold no-print"
                                    title="Open protocol"
                                >
                                    📖
                                </button>
                            ...
                            <button
                                type="button"
                                onClick={() => removeId(id)}
                                className="text-slate-400 hover:text-red-600 font-black no-print"
...
export const PlateTestRenderer = ({
    ...
    const selectedCompounds = useMemo(() => {
        const fromArray = Array.isArray(activeTest.compoundsSelected)
            ? activeTest.compoundsSelected.map((x) => String(x).trim()).filter(Boolean)
            : [];

        if (fromArray.length) return Array.from(new Set(fromArray));

        return compound
            ? Array.from(new Set(compound.split(',').map((s) => s.trim()).filter(Boolean)))
            : [];
    }, [activeTest.compoundsSelected, compound]);

    const updateSelectedCompounds = (next) => {
        const clean = Array.from(new Set((next || []).map((x) => String(x).trim()).filter(Boolean)));
        updatePlate({
            compoundsSelected: clean,
            compound: clean.join(', ')
        });
    };

    const addCustomCompound = (name) => {
        const t = (name || '').trim();
        if (t && !(allCmpds || []).includes(t)) {
            setCustomCmpds((p) => (Array.isArray(p) ? [...p, t] : [t]));
        }
    };

    const linkedProtocolIds = useMemo(() => {
        const fromArray = Array.isArray(activeTest.linkedProtocolIds)
            ? activeTest.linkedProtocolIds.filter(Boolean)
            : [];

        if (fromArray.length) return fromArray;

        return linkedProtocolId ? [linkedProtocolId] : [];
    }, [activeTest.linkedProtocolIds, linkedProtocolId]);

    const updateLinkedProtocolIds = (ids) => {
        const clean = Array.from(new Set((ids || []).filter(Boolean)));
        updatePlate({
            linkedProtocolIds: clean,
            linkedProtocolId: clean[0] || ''
        });
    };

    const promptAddImageLinks = () => {
    ...
    const importImageFiles = async (fileList) => {
        ...
        try {
            ...
            updatePlate({ images: [...displayImages, ...imported] });
        } catch (e) {
            console.error(e);
            ...
            if (dragMode === 'selecting' && tableView === 'region' && activeSel) {
                ...
                for (let r = activeFill.minR; r <= activeFill.maxR; r++) {
                    for (let c = activeFill.minC; c <= activeFill.maxC; c++) {
                        const srcR =
                            activeSel.minR +
                            ((r - activeSel.minR) % (activeSel.maxR - activeSel.minR + 1));
                        const srcC =
                            activeSel.minC +
                            ((c - activeSel.minC) % (activeSel.maxC - activeSel.minC + 1));
    ...
    const autoTouchSD = (region, name) => {
        ...
        compData.vPts.forEach((pt) => {
            const yFit =
                100 / (1 + Math.pow(pt.realX / compData.fit.ic50, compData.fit.hill));
            ...
            for (let it = 0; it < 20; it++) {
                ...
                Object.values(byC).forEach((cps) => {
                    ...
                    if (useFixedSD) sd = fSD;
                        ...
                        sd = Math.sqrt(
                            cps.reduce((s, p) => s + Math.pow(p.y - mean, 2), 0) / (cps.length - 1)
                        );
                    }
    ...
    return (
        <div
            id={`report-container-${activeTest.id}`}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="flex flex-col gap-1 col-span-1 md:col-span-2 lg:col-span-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <label className="text-xs font-bold text-blue-800 uppercase">
                                Compound / Sample Label
                            </label>

                            <CompoundMultiSelect
                                allCompounds={allCmpds || []}
                                selected={selectedCompounds}
                                onChange={updateSelectedCompounds}
                                onAddCustom={addCustomCompound}
                                placeholder="Search or type a compound/sample name..."
                            />
                        </div>
                        ...
                        <div className="flex-1 flex flex-col h-full min-h-[160px]">
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col gap-2">
                                ...
                                <ProtocolMultiSelect
                                    ...
                                    selectedIds={linkedProtocolIds}
                                    onChange={updateLinkedProtocolIds}
                                    onJump={jumpToProtocol}
                                />
                            </div>
                        ...
                        >
                            <div className="w-full flex flex-col items-end">
                                <label className="text-xs font-bold text-slate-600 mb-2 w-full text-right">
                                    🖼️ Images (Import / Links)
                                </label>

                                <div
                                    className="w-full flex flex-col gap-3 border-2 border-dashed border-slate-200 rounded-lg p-2 bg-slate-50/40"
                                    onDragOver={(e) => e.preventDefault()}
                                    onDrop={(e) => {
                                        e.preventDefault();
                                        importImageFiles(e.dataTransfer.files);
                                    }}
                                >
                                    ...
                                    <div className="flex flex-wrap justify-end gap-2 no-print">
                                        <button
                                            ...
                                            className="flex items-center justify-center bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg text-emerald-700 font-bold px-3 py-2 text-xs shadow-sm"
                                        >
                                            📁 Import Images
                                        </button>

                                        <button
                                            onClick={promptAddImageLinks}
                                            className="flex items-center justify-center bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg text-blue-600 font-bold px-3 py-2 text-xs shadow-sm"
                                        >
                                            🔗 Add Image Links
                                        </button>
                                    </div>
                            ...
                            <div className="w-full flex flex-col items-end border-t border-slate-200 pt-3">
                                    ...
                                    {documents.map((doc) => (
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className="text-slate-400 hover:text-red-500 font-bold px-1 opacity-0 group-hover:opacity-100 no-print"
                                            >
                                                ×
                                            </button>
                                        </div>
                                ...
                                <label
                                    className="cursor-pointer text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-200 hover:bg-blue-100 px-3 py-1.5 rounded-lg shadow-sm transition-colors w-full text-center no-print"
                                    onClick={() => {
                            ...
                            <div className="flex flex-wrap gap-2 items-center">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        const d =
                                            parseFloat(
                                                document.getElementById(`cc-dil-${activeTest.id}`).value
                                            ) || dFact;
                                            ...
                                            <tr key={rl}>
                                                ...
                                                {COLS.map((col, c) => {
                                                    ...
                                                    if (tableView === 'od') {
                                                        ...
                                                        val =
                                                            cfg.conc !== null && cfg.conc !== undefined
                                                                ? cfg.conc
                                                                : '';
                                    ...
                                    >
                                        ...
                                        {COLS.map((col, c) => {
                                            ...
                                            let bLeft =
                                                c === 0 ||
                                                (cellConfig[r] &&
                                                    (cellConfig[r][c - 1].region || 'Primary') !== reg);
                            ...
                            <div className="p-5 bg-white border border-slate-300 rounded-xl grid grid-cols-2 lg:grid-cols-4 gap-4 shadow-sm">
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            updatePlate({
                                                chartCfg: { ...chartCfg, xAxisLabel: e.target.value }
                                            })
                                        }
                                        className="border border-slate-300 rounded-md p-2 text-sm outline-none focus:border-blue-500"
                                    />
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) =>
                                            updatePlate({
                                                chartCfg: { ...chartCfg, ptStyle: e.target.value }
                                            })
                                        }
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <div className="flex gap-2">
                                        <select
                                            ...
                                            onChange={(e) =>
                                                updatePlate({
                                                    chartCfg: { ...chartCfg, lineStyle: e.target.value }
                                                })
                                            }
                ...
                >
                    <div className="flex flex-col gap-4">
                        <p className="text-sm text-slate-600">
                            Select the data to format and append to the General Comments (which acts as the Lab
                            Notebook entry).
                        </p>
                            ...
                            onClick={() => {
                                ...
                                if (cbCond) {
                                    html += `<p style="font-size: 12px; color: #475569; margin-bottom: 8px;"><b>Compound:</b> ${
                                        compound || 'N/A'
                                    } | <b>Format:</b> ${activePlateDim.rows}x${
                                        activePlateDim.cols
                                    } | <b>Max Conc:</b> ${topConcStr} ${unit} | <b>Dil. Factor:</b> ${
                                        dilFactorStr
                                    } | <b>Ctrl OD:</b> ${ctrlODStr} | <b>Blank OD:</b> ${bgOD.toFixed(
                                        4
                                    )} | <b>Cells/Well:</b> ${cellsSeeded || 'N/A'} | <b>Time:</b> ${
                                        timeBeforeRevelation || 'N/A'
                                    }h</p>`;
                                }
                                ...
                                if (cbIC50 && Object.keys(processedByRegion).length > 0) {
                                    html += `<table style="width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 11px; text-align: left; background: white;">
                                        <tr style="background-color: #f1f5f9;">
                                            <th style="padding: 6px; border: 1px solid #cbd5e1;">Region</th>
                                            <th style="padding: 6px; border: 1px solid #cbd5e1;">Compound</th>
                                            <th style="padding: 6px; border: 1px solid #cbd5e1;">IC50 (${unit})</th>
                                        </tr>`;
                                    ...
                                    Object.entries(processedByRegion).forEach(([reg, comps]) => {
                                        comps.forEach((c) => {
                                            if (c.fit) {
                                                html += `<tr>
                                                    <td style="padding: 6px; border: 1px solid #e2e8f0;">${reg}</td>
                                                    <td style="padding: 6px; border: 1px solid #e2e8f0;"><b>${c.name}</b></td>
                                                    <td style="padding: 6px; border: 1px solid #e2e8f0;">${c.fit.ic50.toFixed(
                                                        3
                                                    )} ± ${(c.fit.se * eScale).toFixed(3)}</td>
                                                </tr>`;
                                            }
                                        });
                                    });
                    ...
                    >
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    onClick={() =>
                                        confirmRegion(document.getElementById('region-name-input').value)
                                    }
