...
/* ============================================================================
   ...
   custom = {
     Toolbar, Compounds, All, Setup, Data, Fitting, FittingErrors,
...
export const MultiSelectDropdown = ({
  ...
  const buttonCls = emerald ? 'border-emerald-300 focus:border-emerald-500' : 'border-blue-300 focus:border-blue-500';
  ...
  const chipCls = emerald ? 'bg-emerald-100 border-emerald-300 text-emerald-900' : 'bg-blue-100 border-blue-300 text-blue-900';
  const chipRemoveCls = emerald ? 'text-emerald-500 hover:text-red-600' : 'text-blue-500 hover:text-red-600';
  ...
  return (
    <div ref={ref} className={`relative flex flex-col gap-1 p-3 border rounded-lg ${boxCls}`}>
      <label className={`text-xs font-bold uppercase flex items-center justify-between mb-2 ${labelCls}`}>
        <span>{label}</span>
        <span className={`text-[9px] px-2 py-0.5 rounded ${badgeCls}`}>Dropdown • Multiple selection</span>
      </label>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div className={`absolute top-full left-3 right-3 mt-1 z-50 bg-white border rounded-lg shadow-xl max-h-56 overflow-y-auto custom-scrollbar ${menuCls}`}>
          {optionsSafe.length === 0 ? (
            <div className="p-3 text-sm text-slate-400 italic">{emptyHint}</div>
          ) : (
            optionsSafe.map((opt) => (
              <label key={opt} className={`flex items-center gap-2 px-3 py-2 cursor-pointer border-b border-slate-100 last:border-b-0 ${optionHoverCls}`}>
                <input type="checkbox" checked={selectedSafe.includes(opt)} onChange={() => onToggle(opt)} className={`w-4 h-4 ${checkboxCls}`} />
                <span className={`text-sm ${selectedSafe.includes(opt) ? optionSelectedCls : 'text-slate-700'}`}>{opt}</span>
              </label>
            ))
          )}
        </div>
      ...
      {selectedSafe.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2">
          {selectedSafe.map((s) => (
            <span key={s} className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold border ${chipCls}`}>
              {s}
              <button type="button" onClick={() => onToggle(s)} className={`font-black ${chipRemoveCls}`} title={`Remove ${s}`}>×</button>
            </span>
          ))}
          {onClear && (
            <button type="button" onClick={onClear} className="text-xs font-bold text-red-500 hover:text-red-700 underline">Clear all</button>
          )}
        </div>
      )}
    </div>
...
export const TestShellRenderer = ({
  ...
  const definitionCategories = Array.isArray(testCategories) ? testCategories.filter(Boolean) : [];
  const fallbackCategories = config.fallbackCategories || config.categories || ['Activity'];
  ...
  const compoundLabel = samplesCfg.compoundLabel || config.samplesLabel || 'Compound / Sample Label(s)';
  const cellLineLabel = samplesCfg.cellLineLabel || 'Cell Lines / Biological Models';

  const selectedCompounds = (() => {
    if (Array.isArray(t.selectedCompounds)) return t.selectedCompounds.filter(Boolean);
    if (Array.isArray(t.compoundsSelected)) return t.compoundsSelected.filter(Boolean);
    if (typeKey !== 'plate' && Array.isArray(t.compounds)) return t.compounds.filter(Boolean);
    if (compound) return String(compound).split(',').map((s) => s.trim()).filter(Boolean);
    return [];
  })();
  ...
  const clearCompounds = () => {
    const payload = { selectedCompounds: [], compoundsSelected: [], compound: '' };
  ...
  const customFieldMatchesTab = (field) => {
    ...
    return scopes.some((scope) => {
      ...
      if (scope === 'plate') return currentTabKey === 'plate' || currentTestType.startsWith('plate');
      return false;
    });
  };
  ...
  const renderCustomMetadataField = (field) => {
    ...
    return (
      <div key={field.id || field.name}>
        ...
        {field.type === 'date' ? (
          <input type="date" value={val} onChange={(e) => handleCustomFieldChange(field.name, e.target.value)} className={cls} />
        ) : field.type === 'number' ? (
          <input type="number" value={val} onChange={(e) => handleCustomFieldChange(field.name, e.target.value)} className={cls} placeholder="Enter value..." />
        ) : field.type === 'select' ? (
          <select value={val} onChange={(e) => handleCustomFieldChange(field.name, e.target.value)} className={cls}>
            <option value="">-- Select --</option>
            {(field.options || []).map((opt) => <option key={opt} value={opt}>{opt}</option>)}
          </select>
        ) : field.type === 'textarea' ? (
          <textarea value={val} onChange={(e) => handleCustomFieldChange(field.name, e.target.value)} className={cls} placeholder="Enter value..." rows={3} />
        ) : (
          <input type="text" value={val} onChange={(e) => handleCustomFieldChange(field.name, e.target.value)} className={cls} placeholder="Enter value..." />
        )}
      </div>
    );
  };
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
        ...
        {(showCompoundsSection || CompoundsSection) && (
          <CollapsibleSection title="Compounds & Biological Models" icon="🧪">
            ...
            {CompoundsSection && (
              <div className={`pt-6 border-t border-slate-200 ${showCompoundsSection ? 'mt-6' : ''}`}>
              ...
              <div className="w-full flex flex-col items-end border-t border-slate-200 pt-3">
                ...
                <div className="flex flex-col gap-1 w-full mb-3 max-h-[140px] overflow-y-auto custom-scrollbar">
                  ...
                  {documents.map((doc, idx) => (
                    <div key={doc.id || idx} className="flex items-center justify-between bg-slate-50 border border-slate-200 p-1.5 rounded-lg shadow-sm group">
                      <div
              ...
              <div className="flex flex-wrap gap-4 border border-slate-200 p-4 rounded-lg bg-white shadow-sm">
                {notebookChecks.map((c) => (
                  <label key={c.id} className="flex items-center gap-2 text-sm font-bold text-slate-700 cursor-pointer hover:text-blue-600">
                    <input type="checkbox" id={`nb-${typeKey}-${c.id}`} defaultChecked className="w-4 h-4 accent-blue-600 cursor-pointer" />
                    {c.label}
                  </label>
                ))}
              </div>
