...
const getActiveShifts = (t) => {
  ...
  const active =
    instances.find((i) => i.id === t.activeInstanceId) || instances[0];
...
const buildNmrNotebookHtml = (checked, ctx) => {
  ...
  if (checked.seq) {
    html += `<p style="font-size: 12px; color: #475569; margin-bottom: 12px;"><b>Sequence:</b> <span style="font-family: monospace; background: #e2e8f0; padding: 2px 4px; border-radius: 4px;">${
...
const NMR_CUSTOM = {
  All: NMRSections.All || null,
  Setup: NMRSections.Setup || null,
  Data: NMRSections.Data || null,
  Fitting: NMRSections.Fitting || null,
  Simulations: NMRSections.Simulations || null,
