    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col gap-6">
                ...
                {/* STRUTTURA CHIMICA SEMPRE VISIBILE (se la sequenza esiste) */}
                {parsedSeq.length > 0 && (
                ...
                <div className="flex flex-col xl:flex-row gap-6">
                    {/* Tabella di Assegnazione Dinamica */}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                        {parsedSeq.map((res, resIdx) => (
                                            <div key={`1h-${resIdx}`} className="border border-slate-200 rounded-lg overflow-hidden shadow-sm h-fit">
                                                <div className="py-2 text-center font-bold text-sm" style={{ backgroundColor: `${res.color}15`, color: res.color, borderBottom: `1px solid ${res.color}30` }}>
                                                    {res.name} ({res.id})
                                                </div>
                                                <table className="w-full text-sm text-left bg-white">
                                                    <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                                                        <tr><th className="px-3 py-2 font-semibold">Atomo</th><th className="px-3 py-2 font-semibold text-center">Shift (ppm)</th></tr>
                                                    </thead>
                ...
                {/* Visualizzazione Opzionale Spettri (I grafici restano sotto toggle, la struttura no) */}
                {showSim && parsedSeq.length > 0 && (
                    <div className="flex flex-col gap-6 bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-inner">
                        <h3 className="text-lg font-black text-slate-800 border-b border-slate-200 pb-2">Spettri Simulati</h3>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <NMRPlotTemplate title="Spectre 1D ¹H (Simulated)" panelId="1D_1H" expandedPanel={expandedPanel} setExpandedPanel={setExpandedPanel}>
                            ...
                            <NMRPlotTemplate title="Spectre COSY (Simulated)" panelId="cosy" expandedPanel={expandedPanel} setExpandedPanel={setExpandedPanel}>
