...
/* ============================================================================
   CDTestRenderer — FULL REWRITE (single file, drop-in replacement)
   ----------------------------------------------------------------------------
   NMR-parity features now included:
   • Compound selection via dropdown multi-select.
   • Multiple linked protocols (linkedProtocolIds) with add / remove / open.
   • Image import with Google Drive / Dropbox URL normalization (SmartImage)
     and a dedicated "CD Spectra Images" section.
   All original functionality preserved. All UI text in English.
   ========================================================================== */
...
// ================= IMAGE URL NORMALIZATION (same as NMR renderer) =================
...
export const CDTestRenderer = ({
  ...
  // ===== MULTI-PROTOCOL (same as NMR renderer) =====
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
        ...
        <CollapsibleSection title="Comments & Attachments" icon="📝">
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1 flex flex-col h-full min-h-[160px]">
              ...
              />
              {/* MULTIPLE LINKED PROTOCOLS (same as NMR renderer) */}
                  ...
                  <div className="flex flex-wrap justify-end gap-2 max-w-[280px]">
                    {images.map((imgSrc, idx) => (
                      <div key={idx} className="relative inline-block group">
                        ...
                        >
                          <SmartImage src={imgSrc} alt={`Ref ${idx + 1}`} style={{ maxHeight: '150px', maxWidth: '250px' }} />
                        </a>
                        <button
                ...
                <label
                  ...
                  onClick={() => {
                    ...
                    if (url && url.trim()) {
                      ...
                      updatePlate({
                        documents: [...documents, { id: Date.now().toString(), name, type: 'link', data: url.trim() }]
                      });
                    }
                  }}
                >
        ...
        <CollapsibleSection title="Experimental Conditions" icon="🧪">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            {/* COMPOUND / SAMPLE LABEL — DROPDOWN MULTI-SELECT (same as NMR renderer) */}
            <div
            ...
            >
              ...
              >
                <span className={`truncate ${selectedCompounds.length ? 'font-bold text-blue-900' : 'text-slate-400'}`}>
                  {selectedCompounds.length ? selectedCompounds.join(', ') : 'Select compound(s)...'}
                </span>
              ...
              {compoundDropdownOpen && (
                <div className="absolute top-full left-3 right-3 mt-1 z-50 bg-white border border-blue-200 rounded-lg shadow-xl max-h-56 overflow-y-auto custom-scrollbar">
                  {(allCmpds || []).length === 0 ? (
                    <div className="p-3 text-sm text-slate-400 italic">No compounds defined. Add them in Definitions & Labels.</div>
                  ) : (
                    (allCmpds || []).map((cmp) => (
                      <label
                        key={cmp}
                      ...
                      >
                        ...
                        />
                        <span className={`text-sm ${selectedCompounds.includes(cmp) ? 'font-bold text-blue-800' : 'text-slate-700'}`}>
                          {cmp}
                        </span>
                      </label>
                    ))
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-4">
              <div>
                ...
                >
                  {['Activity', 'Toxicity', 'Structure', 'Binding', 'Characterization'].map((cat) => (
        ...
        {/* ===== CD SPECTRA IMAGES (same import style as NMR renderer) ===== */}
        <CollapsibleSection title="CD Spectra Images" icon="🖼️" defaultOpen={true}>
          <div className="flex justify-between items-center mb-4 flex-wrap gap-3">
            <p className="text-sm text-slate-500">
              Attach image links (Google Drive/Dropbox supported) for your experimental CD spectra.
            </p>
            <button
              onClick={() => {
