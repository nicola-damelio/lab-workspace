...
if (checked.cond) {
html += `<p style="font-size: 12px; color: #475569; margin-bottom: 8px;"> <b>Experiment Type:</b> ${t.testCategory || 'N/A'} | <b>Operator(s):</b> ${operatorNames || 'N/A'} | <b>Construct:</b> ${samples} | <b>Host:</b> ${hosts} </p> <p style="font-size: 12px; color: #475569; margin-bottom: 8px;"> <b>Culture:</b> ${t.cultureVolume || '—'}${t.cultureVolumeUnit ? ' ' + t.cultureVolumeUnit : ''} in ${t.medium || '—'} (${t.antibiotic || '—'}${t.antibioticUnit ? ' ' + t.antibioticUnit : ''}) | <b>Induction:</b> ${t.inductionMethod || '—'} ${t.iptgConcentration ? `(${t.iptgConcentration}${t.iptgConcentrationUnit ? ' ' + t.iptgConcentrationUnit : ''})` : ''} at OD ${t.inductionOD || '—'}, ${t.inductionTemp || '—'}${t.inductionTempUnit ? ' ' + t.inductionTempUnit : ''} for ${t.inductionDuration || '—'}${t.inductionDurationUnit ? ' ' + t.inductionDurationUnit : ''} (harvest OD ${t.harvestOD || '—'}) | <b>Lysis:</b> ${t.lysisMethod || '—'} in ${t.lysisBuffer || '—'} ${t.proteaseInhibitors ? `(+inhibitors: ${t.proteaseInhibitors})` : ''} </p> <p style="font-size: 12px; color: #475569; margin-bottom: 8px;"> <b>Tag:</b> ${t.proteinTag || '—'} | <b>Protease:</b> ${t.cleavageProtease || '—'} | <b>Column:</b> ${t.columnType || '—'} | <b>Elution:</b> ${t.elutionConditions || '—'} | <b>Storage buffer:</b> ${t.storageBuffer || '—'} </p>`;
}
