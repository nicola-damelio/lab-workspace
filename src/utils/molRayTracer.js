/* Software ray tracer (browser side) — see molRayTracer.worker.js */
...
/** Expand an NGL structure component into ray spheres (bonds = sphere chains). */
...
/** Run the software ray tracer and resolve with a PNG Blob. */
export const rayTraceStructureToBlob = (
  component,
  { width = 1600, fovDeg = 38, shadowSamples = 6, aoSamples = 0, includeHydrogens = true } = {}
) => {
  ...
  console.info('[ray-tracer] scene:', { atoms: n, center: scene.center, radius: scene.radius, dist, image: effWidth + 'x' + effHeight });
  const job = {
    ...
    lightDir: [-0.55, -0.75, -0.4], bg: [248, 250, 252],
    shadowSamples, aoSamples
  };
  const worker = new Worker(new URL('./molRayTracer.worker.js', import.meta.url), { type: 'module' });
  const promise = new Promise((resolve, reject) => {
    worker.onmessage = (ev) => {
      ...
      if (msg.type === 'done') {
        try {
          if (!msg.hits) {
            worker.terminate();
            reject(new Error('Ray tracer produced no hits — the molecule may not be in the auto-framed view. Check the browser console ([ray-tracer] scene …) and try again with a smaller/larger structure, or use 📷 Figure.'));
            return;
          }
    ...
    worker.onerror = () => {
      worker.terminate();
      reject(new Error('Ray-tracing worker crashed'));
    };
    worker.postMessage({ type: 'render', job });
  });
  const cancel = () => {
    try { worker.postMessage({ type: 'cancel' }); } catch { /* ignore */ }
    setTimeout(() => { try { worker.terminate(); } catch { /* ignore */ } }, 60);
  };
