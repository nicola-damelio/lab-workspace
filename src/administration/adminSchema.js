...
export const ADMIN_PAGES = [
  ...
  { id: 'devisBc', label: 'Approbation devis & BC', icon: '📝', superuserOnly: false, kind: 'devisBc',
    blurb: 'Deux sections seulement. « Devis à approuver » : les devis pas encore signés (visibles par les Permanents ; le Gestionnaire et le Responsable d’achats voient toutes les demandes ; les lignes demandées par « Service » sont visibles par tout le monde). « BC à faire et à approuver » : d’un côté les devis SIGNÉS dont le bon de commande reste à faire — on clique leur ligne pour déposer le BC (le devis y est déjà rattaché), de l’autre les BC déposés à approuver. Un devis signé quitte donc « Devis à approuver » et n’en finit qu’à la signature de son BC. Les fichiers sont téléversés dans Budget_labo/<année>/Devis|BC avec un nom qui reprend le N°, les attributs de la ligne ET l’objet (description) après un « _ » à la fin du titre ; l’approbation d’un devis le valide, et celle du BC lié crée la dépense « BC signé » correspondante dans la page Dépenses (catégorie, ligne budgétaire, N° SIFAC/D.A. et date de signature transmis automatiquement). À l’approbation d’un fichier PDF, une copie signée « …_approuvé_signé.pdf » est créée (image de signature du superutilisateur en bas de la dernière page) et devient le fichier officiel — l’original reste intact.',
