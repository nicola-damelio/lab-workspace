...
try {
  ...
  const guards = [
    ...
    [ROOM_SRC.includes('export const storageRefusedText = (res) => {')
      && ROOM_SRC.includes('describeTopConsumers(usage, 4)'),
      '…et il MESURE le magasin au lieu de répéter « ~5 Mo »'],
    [PAGE_SRC.includes('import { storageFreedText, storageRefusedText } from')
      && PROJ_SRC.includes('import { readLocalStoreUsage, storageFreedText, storageRefusedText }'),
      'la liste des projets et la page projet disent la MÊME chose du magasin'],
    [PROJ_SRC.includes('res = saveProjectsRescued(nextProjects, { projectId: prj.id, fields: {} });'),
      'la CRÉATION d’un projet passe par le sauvetage et se VÉRIFIE (le projet neuf ne peut plus se perdre en silence)'],
    [PROJ_SRC.includes("const [storageWarning, setStorageWarning] = useState('');"),
      '…et un refus du magasin se dit aussi sur la LISTE des projets'],
    [PROJ_SRC.includes('const twin = projects.find(')
      && PROJ_SRC.includes('already exists in this dataset'),
      'un projet du même nom est OUVERT au lieu d’être créé en double (le bouton ne reste plus sans effet)'],
    [PROJ_SRC.includes("LE MAGASIN N'A PAS GARDÉ LE PROJET"),
      '…et si le magasin refuse, on RESTE sur la liste (jamais une page « Project not found »)'],
